...
class WalletUtxosListDlg(QDialog, ui_find_coll_tx_dlg.Ui_ListCollateralTxsDlg, wnd_utils.WndUtils):
    ...
    def display_title(self):
        if len(self.utxos):
            if self.read_only:
                msg = f'<span>Found {self.utxo_value_to_find} Dash transaction(s):</span>'
            else:
            ...
            self.lblMessage.setText(f'<span style="color:red"><b>Found no unspent {self.utxo_value_to_find} Dash '
                                    f'transactions in your wallet</b></span>')
            self.lblMessage.setVisible(True)
